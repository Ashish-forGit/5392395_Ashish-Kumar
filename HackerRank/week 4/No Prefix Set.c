...
// Insert word into Trie and check for prefix violation
bool insertAndCheck(TrieNode* root, char* word) {
    ...
    for (int i = 0; word[i] != '\0'; i++) {
        ...
        // If we reach a word end inside another word
        if (current->isEndOfWord) {
            return false;
        }
    }

    // If current already has children, new word is a prefix of an existing word
    for (int i = 0; i < 26; i++) {
