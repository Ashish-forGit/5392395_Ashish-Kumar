...
int* waiter(int number_count, int* number, int q, int* result_count) {
    ...
    int* result = malloc(number_count * sizeof(int) * (q + 1));
    *result_count = 0;

    int* current = malloc(number_count * sizeof(int));
    int current_count = number_count;
    ...
    for (int i = 0; i < q; i++) {
        int prime = primes[i];
        int* next = malloc(current_count * sizeof(int));
        int next_count = 0;

        int* divisible = malloc(current_count * sizeof(int));
        int div_count = 0;

        for (int j = current_count - 1; j >= 0; j--) {
            if (current[j] % prime == 0) {
                divisible[div_count++] = current[j];
            } else {
                next[next_count++] = current[j];
            }
        }

        for (int j = div_count - 1; j >= 0; j--) {
            result[(*result_count)++] = divisible[j];
        }
